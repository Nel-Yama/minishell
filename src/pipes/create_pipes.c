...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/11/25 23:33:05 by nel-yama          #+#    #+#             */
/*   Updated: 2025/11/30 23:51:06 by nel-yama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	create_pipes(t_arg *arg)
{
	...
	{
		free_and_return(arg);
		exit_error(arg, "minishel: malloc failed");
	}
	i = 0;
	while (i < arg->cmd_count - 1)
	{
		...
		{
			free_and_return(arg);
			exit_error(arg, "minmishel: Failed to create pipe");
		}
		i++;
	}
}
...
int	create_child_process(t_arg *arg)
{
	...
	while (i < arg->cmd_count)
	{
		...
		arg->pid[i] = pid;
		if (pid == 0)
			break ;	// Child process breaks the loop here
			// run_child_for_index(i, envp);
